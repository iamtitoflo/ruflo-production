...
const handlers = {
  'route': () => {
    ...
    if (router && router.routeTask && router.recommendModel) {
      const result = router.routeTask(prompt);
      const modelRec = router.recommendModel(prompt);
      const complexityLabel = modelRec.score <= 30 ? 'LOW' : modelRec.score <= 70 ? 'MEDIUM' : 'HIGH';
      // Emit model recommendation tag — Claude Code injects this into Claude's context
      const output = [
        `[TASK_MODEL_RECOMMENDATION: ${modelRec.model}]`,
        `[INFO] Routing task: ${prompt.substring(0, 80) || '(no prompt)'}`,
        '',
        `Model Routing (complexity score: ${modelRec.score}/100 → ${complexityLabel})`,
        `  - Recommended model: ${modelRec.model.toUpperCase()} (${modelRec.id})`,
        `  - Agent: ${result.agent}`,
        `  - Confidence: ${(result.confidence * 100).toFixed(0)}%`,
        '',
        'Tier guide:',
        '  haiku  (≤30) — conversación simple, preguntas, status',
        '  sonnet (≤70) — implementación, planificación, código',
        '  opus   (>70) — arquitectura, seguridad, análisis complejo',
      ];
      console.log(output.join('\n'));
    } else if (router && router.routeTask) {
      // Fallback: no model routing, default haiku
      const result = router.routeTask(prompt);
      console.log('[TASK_MODEL_RECOMMENDATION: haiku]');
      console.log(`[INFO] Agent: ${result.agent} | Model: haiku (default)`);
    } else {
      console.log('[TASK_MODEL_RECOMMENDATION: haiku]');
      console.log('[INFO] Router not available, using haiku (default)');
    }
  },
